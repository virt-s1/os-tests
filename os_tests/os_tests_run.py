...
def main():
    ...
    if args.image is not None:
        if 'azure' in args.image:
            ...
            print("only azure and rhel-kvm image check supported for now")
            sys.exit(0)
    else:
        print("skip azure and rhel-kvm image check by default")
        if skip_patterns and not args.verifydoc:
            skip_patterns = skip_patterns + ',test_azure_image,test_rhel_guest_image'
        else:
            if not args.verifydoc:
                skip_patterns = 'test_azure_image,rhel_guest_image'
